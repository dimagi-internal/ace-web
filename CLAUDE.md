...
- **Observability** uses AWS-native tooling: structured JSON logs to stdout (captured by ECS `awslogs` driver → CloudWatch Logs Insights), X-Ray distributed tracing (requires an `xray-daemon` ECS sidecar in `deploy/aws/task-definition.json`). `RequestIDFilter` stamps each log record with a `request_id`; `RequestIDMiddleware` generates or propagates `X-Request-ID` headers. No third-party SaaS token required.
