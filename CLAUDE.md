...
- **API modernization Phases 0-8 are DONE** (merged 2026-05-14): Django Ninja, Pydantic v2, OpenAPI 3.1, RFC 7807 problem+json, Scalar/Redoc, generated frontend types, Schemathesis CI contract tests, FastMCP bridge, basedpyright CI, Logfire observability wiring. No further API modernization work is planned.
- No eval harness, no security review — see **Phase 5 deferred** note below.
- **Logfire observability** is wired but inactive: add `LOGFIRE_TOKEN` to AWS Secrets Manager + `deploy/aws/task-definition.json` to activate request/Pydantic/httpx tracing. No code change needed.
