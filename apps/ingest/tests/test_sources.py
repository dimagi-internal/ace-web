...
def test_a_local_session_never_calls_canopy_even_when_a_turn_id_exists():
    ...
    with mock.patch("apps.canopy.client.visitor_token") as exchange:
        assert sources.session_raw_jsonl(s) == LINE_A
...
@override_settings(**ON)
def test_an_owner_with_no_email_fetches_nothing_rather_than_borrowing_an_identity():
    """There is no fallback identity: a read on behalf of nobody is refused,
    and the cost reads as missing — never as someone else's transcript."""
    user, s = _session(canopy_session_id="sess-1")
