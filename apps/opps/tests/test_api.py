...
# ---------------------------------------------------------------------------
# Reviewer feedback — GET /w/{ws}/opps/{slug}/feedback
# ---------------------------------------------------------------------------
    ...
    "records": [
        {
            ...
            "items": [{"id": "d", "anchor": "§5 Visit flow", "verbatim": "visit_outcome first"}],
            "item_count": 1,
            "tally": {"comments": 9, "shipped": 8, "needs_human": 1, "unrouted": 0},
            "ledger_body": "9 comments — 8 shipped, 1 need a human, 0 unrouted.",
            "ledger_url": "u/ledger",
            "record_url": "u/record",
        }
    ],
...
@pytest.mark.django_db
def test_get_opp_feedback_happy_path(member_client, monkeypatch):
    client, _, _ = member_client
    monkeypatch.setattr(
        "apps.opps.api.load_feedback_payload", lambda workspace, slug: _FAKE_FEEDBACK
    )
    response = client.get("/api/w/ws1/opps/opp-1/feedback")
    assert response.status_code == 200
    body = response.json()
    assert body["records"][0]["reviewer"] == "Sophie Feintuch"
    assert body["records"][0]["tally"]["shipped"] == 8


@pytest.mark.django_db
def test_get_opp_feedback_etag_round_trips_304(member_client, monkeypatch):
    client, _, _ = member_client
    monkeypatch.setattr(
        "apps.opps.api.load_feedback_payload", lambda workspace, slug: _FAKE_FEEDBACK
    )
    first = client.get("/api/w/ws1/opps/opp-1/feedback")
    again = client.get(
        "/api/w/ws1/opps/opp-1/feedback", HTTP_IF_NONE_MATCH=first["ETag"]
    )
    assert again.status_code == 304


@pytest.mark.django_db
def test_get_opp_feedback_empty_is_200_not_404(member_client, monkeypatch):
    """An opp nobody has reviewed is a normal state, not a missing page."""
    client, _, _ = member_client
    monkeypatch.setattr(
        "apps.opps.api.load_feedback_payload",
        lambda workspace, slug: {"schema_version": 1, "records": []},
    )
    response = client.get("/api/w/ws1/opps/opp-1/feedback")
    assert response.status_code == 200
...
@pytest.mark.django_db
def test_get_opp_feedback_404_unknown_opp(member_client, monkeypatch):
    client, _, _ = member_client
    monkeypatch.setattr("apps.opps.api.load_feedback_payload", lambda workspace, slug: None)
    assert client.get("/api/w/ws1/opps/nope/feedback").status_code == 404


@pytest.mark.django_db
def test_get_opp_feedback_404_non_member(non_member_client):
    client, _, _ = non_member_client
    assert client.get("/api/w/ws1/opps/opp-1/feedback").status_code == 404
