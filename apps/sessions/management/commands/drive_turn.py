...
so a programmatically-created run executes through the turn-driver +
channel-layer broadcast path (``apps.sessions.turn_driver.drive_and_
broadcast``), decoupled from the web request's event loop (a fire-and-forget
asyncio task spawned inside a Django async request does not reliably run;
see ace-web#585). ``claude -p`` then spawns cleanly as this process's own
subprocess.
"""
