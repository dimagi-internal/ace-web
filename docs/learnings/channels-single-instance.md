...
Before raising the ECS desired-count above 1, Plan 1C (or whoever scales the service) must:

1. Add `channels-redis` to `pyproject.toml`
2. Point `CHANNEL_LAYERS` in `config/settings/production.py` at the shared connect-labs AWS ElastiCache Redis instance (free — already provisioned, no new infra required)
3. Replace `CHANNEL_LAYERS` in `config/settings/production.py` with a `channels_redis.core.RedisChannelLayer` pointing at the ElastiCache instance
4. Raise the ECS desired-count

Do not increase the task count until all steps are done. A prominent WARNING comment already exists in `config/settings/production.py` — keep it synchronized with the real state.
