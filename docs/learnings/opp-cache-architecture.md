...
**Bump `_KEY_VERSION` *also* when the file_id-tracking semantics change**, not only when dataclass shapes change. The cache invalidates entries whose tracked file_ids intersect Drive's changes feed; if a deploy adds new IDs to the tracking set (e.g. parent folder IDs in #277, so that a new child appearing under a tracked folder triggers invalidation via the parent's bumped `modifiedTime`), pre-deploy entries still carry the old, narrower set and never receive the new signals. Symptom: header counts/timestamps stay stale forever for opps that existed before the deploy, while newly-touched opps refresh fine. v2 → v3 in #279 covered this case. Rule of thumb: any change to what `TouchedFileTracker` records — or to which folders/files a `load_opp` / `load_opp_card` walk visits — needs a `_KEY_VERSION` bump.
